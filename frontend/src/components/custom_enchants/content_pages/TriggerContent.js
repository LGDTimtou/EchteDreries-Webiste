...
import {conditionsToParameters} from "../../../data/trigger_conditions/parameters";
...
const TriggerContent = ({category, triggerName, trigger}) => {
    ...
    const conditionsParameters = conditionsToParameters(trigger.trigger_conditions)
                        ...
                        {trigger.trigger_conditions.map(triggerConditionParent => (
                            <div>
                                <p className="subsubsection-title offset">
                                    {toTitleCase(combineStrings("_", triggerConditionParent.prefix, triggerConditionParent.group))}
                                </p>
                                {
                                    triggerConditionGroups[triggerConditionParent.group].values.map(triggerCondition => {
                                        const name = combineStrings("_", triggerConditionParent.prefix, triggerConditionParent.group, triggerCondition.suffix)
                                        return (
                                            <div className="parameter-item">
                                    <span
                                        className="parameter-name">{toTitleCase(name)}:
                                    </span>
                                                <span className="parameter-description">
                                        {triggerCondition.description + triggerConditionParent.description} => %{name}%
                                    </span>
                                            </div>
                                        )
                                    })
